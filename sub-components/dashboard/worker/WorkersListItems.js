...
const formatAddress = (address) => {
  ...
  // Custom format
  return [streetAddress, city, stateProvince, postalCode, country]
    .filter(Boolean) // Remove empty values
    .join(', ');
};

const FilterPanel = ({ filters, setFilters, onClear, loading, handleSearch }) => {
...
const WorkersListItems = () => {
  const { workers, loading, error, fetchWorkers, clearCache } = useWorkers();
  ...
  const handleSearch = useCallback(() => {
    ...
    if (filters.status) {
      ...
      filtered = filtered.filter((worker) => worker.activeUser === isActive);
    }
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    const confirmDelete = await Swal.fire({
      ...
      text: `Are you sure you want to delete ${selectedRows.length} worker${
    ...
    if (confirmDelete.isConfirmed) {
      try {
        ...
        toast.success(
          `Successfully deleted ${selectedRows.length} worker${selectedRows.length > 1 ? 's' : ''}`,
          {
            position: 'top-right',
            className: 'bg-success text-white',
          }
  ...
  const calculateStats = useCallback(() => {
    ...
    const active = workers.filter((worker) => worker.activeUser).length;
    const inactive = workers.filter((worker) => !worker.activeUser && worker.id !== 'Admin').length;
