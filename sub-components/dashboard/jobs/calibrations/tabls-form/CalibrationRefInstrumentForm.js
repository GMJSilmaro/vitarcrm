...
const CalibrationRefInstrumentForm = ({ job, data, isLoading, handleNext, handlePrevious }) => {
  ...
  useEffect(() => {
    ...
    //TODO: Optimize the query to use firebase 'where' clause and 'in' operator

    const eqIds = job?.data?.equipments?.map((equipment) => equipment.id) || [];
    const q = query(collection(db, 'equipments'), where('inventoryId', 'in', eqIds));
    ...
    getDocs(q)
      .then((snapshot) => {
        if (!snapshot.empty && !isMounted.current) {
          ...
          setInstruments({
            data: snapshot.docs.map((doc) => {
              const instrumentData = doc.data();

              return {
                id: doc.id,
                tagId: instrumentData?.tagId,
                description: instrumentData?.description,
                category: instrumentData.category?.toLowerCase() || '',
                make: instrumentData?.make,
                model: instrumentData?.model,
                serialNumber: instrumentData?.serialNumber,
                traceability: instrumentData?.traceability,
                certificateNo: instrumentData?.certificateNo,
                uncertainty: instrumentData?.uncertainty,
                uncertaintyUnit: instrumentData?.uncertaintyUnit,
        ...
        setInstruments({ data: [], isLoading: false, isError: false });
      })
      .catch((err) => {
  ...
  }, [form.watch('category'), JSON.stringify(instruments)]);
  ...
  }, [data, JSON.stringify(instruments), table]);
...
export default CalibrationRefInstrumentForm;
