...
const WBCalculationTable = ({ rangeIndex }) => {
...
const RefTable = ({ dfnv, pointIndex, showUncertaintyBudget, rtest, etest, rangeIndex }) => {
  ...
  const mpe = useWatch({control: form.control, name: 'mpe'}) || { data: [], isLoading: true, isError: false }; //prettier-ignore
  ...
  }, [JSON.stringify(cuswd), JSON.stringify(mpe), JSON.stringify(entryData)]);

  // useEffect(() => {
  //   console.log({
  //     category: 'mass',
  //     variant: 'weight-balance',
  //     entryData,
  //     refTableData,
  //     cuswd,
  //     pointIndex,
  //   });
  // }, [JSON.stringify(entryData), JSON.stringify(refTableData), JSON.stringify(cuswd), pointIndex]);
...
const UncertaintyBudgetTable = ({ pointIndex, calculations, rangeIndex }) => {
  ...
  const resolutionWithoutOrWithLoadCiUi4v = useMemo(() => {
    const x = commonValueV;
    const y = 1;
    const z = multiply(resolutionWithoutOrWithLoadUi, y);
    const a = pow(z, 4);

    return divide(a, x);
  }, [commonValueV, resolutionWithoutOrWithLoadUi]);
  ...
  const eccentricityTestCiUi4v = useMemo(() => {
    ...
    return divide(a, x);
  }, [calculations.resolution, commonValueV, eccentricityTestUi]);
  ...
  const driftCiUi4v = useMemo(() => {
    ...
    return divide(a, x);
  }, [calculations.resolution, commonValueV, driftUi]);
      ...
      <Table className='text-center fs-6 m-0 align-middle' bordered responsive>
        ...
        <tbody>
          <tr>
            ...
            <td>
              {format(balanceReadingPercent * 100, {
                notation: 'fixed',
                precision: 2,
              })}
              %
          ...
          <tr>
            ...
            <td>B</td>
            <td></td>
            <td>{formatScientific(commonValueV, 1)}</td>
          ...
          <tr>
            ...
            <td>
              {format(weightStandardPercent * 100, {
                notation: 'fixed',
                precision: 2,
              })}
              %
          ...
          <tr>
            ...
            <td>
              {format(repeatabilityTestPercent * 100, {
                notation: 'fixed',
                precision: 2,
              })}
              %
          ...
          <tr>
            ...
            <td>{calculations.resolution ? formatScientific(commonValueV, 1) : ''}</td>
            ...
            <td>
              {format(eccentricityTestPercent * 100, {
                notation: 'fixed',
                precision: 2,
              })}
              %
...
export default WBCalculationTable;
