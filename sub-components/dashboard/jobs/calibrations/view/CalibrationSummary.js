...
const CalibrationSummary = ({ calibration }) => {
  ...
  const category = useMemo(() => {
    const value = calibration?.category || '';
    return value
      .split(' ')
  ...
  const variant = useMemo(() => {
    const value = calibration?.variant || '';
    return value
      .split('-')
      .map((str) => _.capitalize(str))
      .join(' ');
        ...
        <Row className='row-gap-3'>
          ...
          <Col md={3}>
            <div className='d-flex align-items-sm-center gap-3 p-3 bg-light-subtle rounded border border-light-subtle w-100 h-100'>
              ...
              <div>
                <div className='text-secondary fs-6'>Variant:</div>
                <div className='text-primary-label fw-semibold text-capitalize'>
                  {variant || 'N/A'}
                </div>
              </div>
          ...
          <Col md={4}>
            <div className='d-flex align-items-sm-center gap-3 p-3 bg-light-subtle rounded border border-light-subtle w-100 h-100'>
              ...
              <div>
                ...
                <div className='text-primary-label fw-semibold text-capitalize'>
                  {calibration?.dueDateRequested && isValid(new Date(calibration?.dueDateRequested))
                    ? format(calibration?.dueDateRequested, 'dd/MM/yyyy')
                    : 'N/A'}
                </div>
              </div>
            </div>
