        ...
        <div className='d-flex justify-content-between align-items-center mb-2'>
          <div className='d-flex align-items-center flex-grow-1'>
            <OverlayTrigger
              ...
              overlay={
                <Tooltip>
                  Click to {isExpanded ? 'collapse' : 'expand'} search for customers
                </Tooltip>
              }
            >
              <div
...
const AddressesModal = ({
  ...
  const showBillingOnly =
    filterType === 'billing' || (filterType === 'all' && filteredAddresses.shipping.length === 0);
  const showShippingOnly =
    filterType === 'shipping' || (filterType === 'all' && filteredAddresses.billing.length === 0);
  ...
  const totalPages = Math.ceil(
    Math.max(
      ...
      showBothColumns
        ? Math.max(filteredAddresses.billing.length, filteredAddresses.shipping.length)
        : 0
    ) / itemsPerPage
  );
  ...
  const AddressTable = ({ addresses, type, billtoDefault, shiptoDefault }) => {
    console.log('AddressTable Props:', {
      ...
      addresses: addresses.map((a) => ({
        ...
        isDefault:
          type === 'shipping' ? a.AddressName === shiptoDefault : a.AddressName === billtoDefault,
      })),
    });
                ...
                <tr key={index}>
                  <td>
                    <div className='d-flex align-items-center'>
                      ...
                      <span className='fw-bold text-primary'>{address.AddressName || '-'}</span>
                    </div>
                  </td>
                  <td>
                    <div className='text-wrap' style={{ maxWidth: '200px' }}>
                      ...
                      {[
                        ...
                        address.BuildingFloorRoom !== address.AddressName
                          ? address.BuildingFloorRoom
                          : null,
                        address.Street,
                        address.ZipCode,
  ...
  return (
    <Modal show={show} onHide={onHide} size='xl' onClick={(e) => e.stopPropagation()}>
      ...
      <Modal.Body onClick={(e) => e.stopPropagation()}>
        ...
        <div className='mb-3 text-muted small d-flex align-items-center'>
          ...
          Found {filteredAddresses.billing.length} billing and {filteredAddresses.shipping.length}{' '}
          shipping addresses
        </div>

        {/* Dynamic Content */}
        <Row>
          {getCurrentPageItems(filteredAddresses.billing).length > 0 &&
            (filterType === 'all' || filterType === 'billing') && (
        ...
        {totalPages > 1 && (
          <div className='d-flex justify-content-between align-items-center mt-4'>
            <div className='text-muted small'>
              ...
              of {Math.max(filteredAddresses.billing.length, filteredAddresses.shipping.length)}
            </div>
            <div className='d-flex align-items-center'>
              <Button
...
const ViewCustomers = () => {
              ...
              overlay={
                <Tooltip>
                  <div className='text-start'>
                    ...
                    <div className='mt-1 text-xs'>Click to see full customer profile</div>
                  </div>
                </Tooltip>
              }
            >
  ...
  const handleClearFilters = async () => {
    ...
    try {
      ...
      const clearQuery = query(customersRef, orderBy('customerId', 'asc'));
  ...
  const QuickActions = ({ customer }) => (
    <DropdownButton size='sm' variant='light' title='Actions'>
      ...
      <Dropdown.Divider />
      <Dropdown.Item className='text-danger' onClick={() => handleDeleteCustomer(customer)}>
  ...
  const handlePerRowsChange = async (newPerPage) => {
    ...
    try {
      ...
      const customersRef = collection(db, 'customers');
      const newQuery = query(customersRef, orderBy('customerId', 'asc'), limit(newPerPage));
  ...
  const handleSearch = async (searchFilters) => {
    ...
    try {
      ...
      if (searchFilters.quickSearch) {
        ...
        let searchQuery = query(customersRef, where('customerName', '==', searchTerm), limit(999));
        ...
        let searchQuery = query(customersRef, limit(10));
        ...
        if (Object.values(searchFilters).some((filter) => filter)) {
          searchResults = searchResults.filter((customer) => {
            if (
              ...
              !customer.customerId?.toLowerCase().includes(searchFilters.customerId.toLowerCase())
            )
              return false;
            if (
            ...
            if (
              ...
              !customer.country?.toLowerCase().includes(searchFilters.country.toLowerCase())
            )
              return false;
            return true;
          });
                ...
                <div className='d-flex gap-2'>
                  ...
                  <div className='d-flex align-items-center gap-1'>
                    {pageNumbers.map((page, index) =>
                      page === '...' ? (
                        <span key={`dot-${index}`} className='px-2 text-muted'>
                          ...
                        </span>
                      ) : (
                        <Button
...
const copyAddressToClipboard = (address, e) => {
  ...
  const formattedAddress = [
    address.AddressName,
    address.BuildingFloorRoom && address.BuildingFloorRoom !== address.AddressName
      ? address.BuildingFloorRoom
      : null,
    address.Street,
    address.ZipCode,
